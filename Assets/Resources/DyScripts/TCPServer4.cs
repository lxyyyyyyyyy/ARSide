...
// Class declaration
[System.Serializable]
public class MyEvent4 : UnityEvent<int> { }

public class TCPServer4 : MonoBehaviour
{
    private MyEvent4 OnDevicesArrived;

    public GameObject prefabPointCloud;

    private StateObject4 state;
    ...
    void Start()
    {
        ...
        Debug.Log("Server4 host ip is:" + hostip);
        ...
        lock (pointcloud_lock)
        {
            if (devices_parameters_arrived && !pointcloud_instantiated)
            {
                ...
                for (int i = 0; i < n_devices; i++)
                {
                    ...
                    if (!FRAME_CREATED)
                    {
                        FRAME = new byte[width * heigth * (3 + 2) + StateObject4.BufferSize];
                        FRAME_CREATED = true;
                    }
    ...
    public void AcceptCallback(IAsyncResult ar)
    {
        ...
        state = new StateObject4();
        state.workSocket = handler;
        handler.BeginReceive(state.buffer, 0, StateObject4.BufferSize, 0,
    ...
    private void ReadCallback(IAsyncResult ar)
    {
        ...
        StateObject4 state = (StateObject4)ar.AsyncState;
